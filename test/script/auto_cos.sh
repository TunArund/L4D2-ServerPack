...
# 函数可加载
check "cos_configured() 可调用" 'docker compose exec -T downloader php -r "
...
check "cos_generate_auth() 可调用" 'docker compose exec -T downloader php -r "
include_once \"/var/www/html/api/tools.php\";
include_once \"/var/www/html/api/cos_client.php\";
exit(function_exists(\"cos_generate_auth\") ? 0 : 1);
"'

check "cos_head_object() 可调用" 'docker compose exec -T downloader php -r "
include_once \"/var/www/html/api/tools.php\";
include_once \"/var/www/html/api/cos_client.php\";
exit(function_exists(\"cos_head_object\") ? 0 : 1);
"'

check "cos_upload_file() 可调用" 'docker compose exec -T downloader php -r "
include_once \"/var/www/html/api/tools.php\";
include_once \"/var/www/html/api/cos_client.php\";
exit(function_exists(\"cos_upload_file\") ? 0 : 1);
"'

check "daily_log_path() 可调用" 'docker compose exec -T downloader php -r "
include_once \"/var/www/html/api/tools.php\";
...
echo ""
echo "[COS 签名格式]"
SIG_TEST=$(docker compose exec -T downloader php -r '
include_once "/var/www/html/api/tools.php";
include_once "/var/www/html/api/cos_client.php";
