...
## 2026-09-06 — map_info 按钮文案语义化

- 地图详情页按钮文案由模糊描述改为明确动作，按「订阅 / 下载」两类区分获取方式
  - 订阅（创意工坊）：「跳转到steam页面」→「在浏览器订阅」、「在steam客户端中查看」→「在Steam客户端订阅」
  - 下载（直链）：「国内直链下载」→「国内CDN直链下载」（含未登录提示文案）、「SteamCDN（直链下载）」→「SteamCDN直链下载」
- 区分国内 CDN（COS）与 Steam 官方 CDN 两条下载线路
